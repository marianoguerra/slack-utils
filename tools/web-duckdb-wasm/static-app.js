...
class DuckDBStaticApp {
    ...
    async loadInitialData() {
        ...
        await Promise.all([
            this.loadStaticFile('users', '/users.parquet'),
            this.loadStaticFile('channels', '/channels.parquet')
        ]);
    ...
    async loadStaticFile(tableName, path) {
    ...
    // Generate all year/week combinations for a date range
    generateWeeksInRange(startDate, endDate) {
        const weeks = [];
        const start = new Date(startDate);
        const end = new Date(endDate);

        // Get start and end ISO weeks
        const startIso = this.getISOWeek(start);
        const endIso = this.getISOWeek(end);

        // Generate all weeks from start to end
        let currentYear = startIso.year;
        let currentWeek = startIso.week;

        while (currentYear < endIso.year || (currentYear === endIso.year && currentWeek <= endIso.week)) {
            weeks.push({ year: currentYear, week: currentWeek });

            // Move to next week
            currentWeek++;

            // Check if we need to move to next year (ISO weeks can be 52 or 53)
            const lastWeekOfYear = this.getISOWeek(new Date(currentYear, 11, 28)).week;
    ...
    async loadConversations() {
        ...
        try {
            // Generate all possible year/week combinations in the date range
            const weeks = this.generateWeeksInRange(startDate, endDate);

            if (weeks.length === 0) {
                statusEl.textContent = 'No weeks in range';
                statusEl.className = 'status-value';
                loadBtn.disabled = false;
                return;
            }

            // Try to fetch each possible parquet file, ignoring 404s
            const fileNames = [];
            let loaded = 0;
            let notFound = 0;
            ...
            for (let i = 0; i < weeks.length; i++) {
                ...
                statusEl.textContent = `Checking ${i + 1}/${weeks.length}...`;
                ...
                try {
                    ...
                    if (response.ok) {
                        const arrayBuffer = await response.arrayBuffer();
                        const fileName = `conv_${year}_${week}.parquet`;
                        await this.db.registerFileBuffer(fileName, new Uint8Array(arrayBuffer));
            ...
            statusEl.textContent = `${count} messages (${loaded} files)`;
            statusEl.className = 'status-value loaded';
        } catch (error) {
